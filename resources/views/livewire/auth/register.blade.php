<main>
    <section class="py-24 w-11/12 lg:w-5/12 mx-auto">
        <div class="rounded-[15px] border border-[#DADADA] p-5 lg:p-10">
            <h6 class="text-xl font-bold">Регистрация</h6>

            <form wire:submit='register()' class="mt-5" method="POST">

                @session('success')
                    <div class="text-blue-500 font-medium text-xs block mb-4">
                        <span>
                            Вы успешно зарегистрировались! Теперь вы можете <a href="{{ route('login') }}"
                                class="underline">войти</a> в свой
                            аккаунт.
                        </span>
                    </div>
                @endsession
                ...
                <div class="flex items-center space-x-5 mt-5">
                    <button type="submit"
                        class="bg-[#C7A771] py-2.5 px-5 text-white transition duration-500 hover:bg-[#A88448] rounded-[6px] text-black text-sm lg:text-[14px] font-bold text-center">Зарегистрироваться</button>
                    <a href="{{ route('login') }}" class="text-sm">Авторизоваться</a>
                </div>
            </form>
        </div>
    </section>
